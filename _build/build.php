...
class modExtraPackage
{
    ...
    public function __construct($core_path, array $config = [])
    {
        ...
        require_once dirname(__FILE__) . '/includes/encpackagebuilder.class.php';
        $this->initialize();
    }
    ...
    {
        $isEncrypt = $this->config['encrypt'];
        if ($isEncrypt) {
            $this->builder = new encPackageBuilder($this->modx, $this->config);
        } else {
